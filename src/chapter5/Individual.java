	...
	public Individual(Timetable timetable) {
		...
		for (Group group : timetable.getGroupsAsArray()) {
			...
			for (int moduleId : group.getModuleIds()) {
				...
				int moduleDuration = timetable.getModule(moduleId).getDuration();
				
				// Fill the module with enough timeslots as its duration
				for (int x = 0; x < moduleDuration; x++) {
					...
					// Add random times to the module
