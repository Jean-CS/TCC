...
public class Timetable {
	 ...
	 * @param professorIds
	 */
	public void addModule(int moduleId, String moduleCode, String module, int duration, int professorIds[]) {
		this.modules.put(moduleId, new Module(moduleId, moduleCode, module, duration, professorIds));
	}
	...
	public void createClasses(Individual individual) {
		...
		for (Group group : this.getGroupsAsArray()) {
			...
			for (int moduleId : moduleIds) {
				int moduleDuration = getModule(moduleId).getDuration();
				classes[classIndex] = new Class(classIndex, group.getGroupId(), moduleId, moduleDuration);

				int[] s = new int[moduleDuration];
				for (int x = 0; x < moduleDuration; x++) {
					// Add professor
					classes[classIndex].addProfessor(chromosome[chromosomePos]);
					chromosomePos++;
					s[x] = chromosome[chromosomePos++];
				}
				
				// Add timeslot
				classes[classIndex].addTimeslot(s);
...
//				if (classA.getClassId() != classB.getClassId()) {
					// Check professor on same class
					if (classA.getProfessorId() == classB.getProfessorId()) clashes++;
					
					int[] timeslotsA = classA.getTimeslotId();
					int[] timeslotsB = classB.getTimeslotId();
					
					int lengthA = timeslotsA.length - 1;
					int lengthB = timeslotsB.length - 1;
			
					if (lengthA >= lengthB) {
						for (int i = 0; i <= lengthA; i++) {
							for (int j = 0; j <= lengthB; j++) {
								try {
									if (timeslotsA[i] == timeslotsB[j]) clashes++;
								} catch (Exception e) {
									System.out.println("i = " + i + "\nj = " + j + "\nA = " + timeslotsA.toString() + "\nB = " + timeslotsB.toString());
									break;
								}
							}
						}
						...
						for (int i = 0; i <= lengthB; i++) {
							for (int j = 0; j <= lengthA; j++) {
								try {
									if (timeslotsB[i] == timeslotsA[j]) clashes++;
								} catch (Exception e) {
									System.out.println("i = " + i + "\nj = " + j + "\nA = " + timeslotsA.toString() + "\nB = " + timeslotsB.toString());
									break;
								}
